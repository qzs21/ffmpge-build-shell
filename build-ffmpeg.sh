...
tar zxf "${SRCDIR}/ffmpeg-${VERSION}.tar.bz2" -C $SRCDIR
...
do
    ...
    make && echo "---*** make finished : ${ARCH} ! ***---" && make install && echo "---*** installed : ${ARCH} ! ***---" && make clean
...
xcrun   -sdk iphoneos lipo -output universal/lib/$file \
        ...
        -arch arm64 arm64/lib/$file \
        -arch i386 i386/lib/$file \
        -arch x86_64 x86_64/lib/$file
